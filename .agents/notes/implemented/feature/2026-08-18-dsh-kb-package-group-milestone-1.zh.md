...
单包会随模块稳定前持续变大，卡片模型里的中文属性名在本仓库也少见——两者都是有意为之（启动文档决策 1 与 7.1）。检索每次同步会重新解析全部卡片文件；在里程碑 1 规模下可接受，已记为限制。P0/P1 层级经 `kb_write` 接受任意卡片形态；层级语义（Inbox 周清空、项目笔记）是里程碑 1 之外的产品工作。团队库、知识包与注入（`kb/injected`）、治理、记账、复盘、Web 工作台、MCP 均按路线图推迟，`ctx.kb` 接缝、`kb/*` 事件与 `kb-search` 接口正是这些里程碑要基于的扩展点。验收链——写入、检索、晋升、回放——在真实 workspace 内可跑通，且不改 agent-loop。
