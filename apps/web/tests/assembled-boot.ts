...
// The kb workbench ships outside the shipped web-app bundle (an opt-in overlay,
// examples/kb-web/cordis.yml); its snapshot lane mounts the same overlay the
// product documents, so the assembled graph matches a real `dsh web --patch`.
const OPT_IN_LAYERS = [
  {
    manifest: join(REPO_ROOT, 'packages/bundle/web-app/package.json'),
    patch: join(REPO_ROOT, 'examples/kb-web/cordis.yml'),
  },
] as const
const ASSEMBLED_LAYERS = [...BUNDLE_LAYERS, ...OPT_IN_LAYERS] as const
const bundleResolvers = ASSEMBLED_LAYERS.map(layer => createRequire(layer.manifest))
...
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  const entries = appBoot.composeEntries(ASSEMBLED_LAYERS.map(layer =>
