...
import { KB_PACK_SECTION, KB_PACK_SECTION_ORDER, foldInjected, hasInjectedPack, renderCardSection, selectPackCards } from './pack.ts'
 ...
 * the library through the store's sync path. Each pack injects once per
 ...
 * logs and continues without breaking agent publication.
...
export function injectPacks(ctx: Context, kb: KbService, agent: Agent): void {
  ...
  let listed
  try {
    listed = new PersonalCardStore(cwd, kb.config.cardsPath).listSync()
  } catch (error) {
    ctx.logger.warn(`${PLUGIN}: failed to read the library at session start for agent "${agent.id}": %o`, error)
    return
  }
  ...
  for (const pack of kb.config.packs) {
    try {
      ...
      const selected = selectPackCards(listed.cards, pack)
